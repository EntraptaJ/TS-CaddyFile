...
import { HTTPDirectiveTypes, DirectiveTypes, CaddyFile, HTTPDirectiveTypesType, BINDDirective } from './types';
...
// @ts-ignore
const getLayer = (CaddyFile: CaddyFile, path: string[]): any => path.reduce((obj, prop) => obj[prop], CaddyFile);
...
function extractDirective<T extends HTTPDirectiveTypesType>(directive: T, directiveString: string): DirectiveTypes {
  if (directive === 'basicauth') return processBasicAuthDirective(directiveString);
    ...
    return { type: 'bind', ...((/(?!.*bind)\s(?<host>.*)/.exec(directiveString).groups as unknown) as BINDDirective) };
}
  ...
  for (const line of lines) {
    if (commentTest.test(line)) {
      ...
      getLayer(CaddyFileOBJ, path).directives.push(
        extractDirective(directiveTest.exec(line)[0] as keyof typeof HTTPDirectiveTypes, line)
      );
    } else if (directiveTest.test(line) && bracketTest.test(line)) directive = line;
    else if (/\S+/.test(line) && directive) {
      directive = `${directive}\n${line.replace(/(\s|\t){2,}/g, '')}`;
      if (closeBracketTest.test(line)) {
        getLayer(CaddyFileOBJ, path).directives.push(
          extractDirective(directiveTest.exec(directive)[0] as keyof typeof HTTPDirectiveTypes, directive)
        );
        directive = undefined;
      }
